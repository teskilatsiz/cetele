...
function formatPublicKey(publicKey?: string): string {
  if (!publicKey || publicKey.startsWith('npub1')) return publicKey || '';
  if (!/^[0-9a-f]{64}$/i.test(publicKey)) return publicKey;

  try {
    return nip19.npubEncode(publicKey);
  } catch (error) {
    console.warn('Public key could not be encoded as npub:', error);
    return publicKey;
  }
}
...
export default function SettingsScreen() {
  ...
  const transparentBackground = scheme === 'light'
    ? 'rgba(245,246,248,0)'
    : 'rgba(0,0,0,0)';
  ...
  const compactHeaderStyle = useAnimatedStyle(() => ({
    opacity: interpolate(scrollY.value, [32, 76], [0, 1], Extrapolation.CLAMP),
    transform: [{
      translateY: interpolate(scrollY.value, [32, 76], [10, 0], Extrapolation.CLAMP),
    }],
  }));
  const largeTitleStyle = useAnimatedStyle(() => ({
    opacity: interpolate(scrollY.value, [0, 56], [1, 0], Extrapolation.CLAMP),
    transform: [{
      translateY: interpolate(scrollY.value, [0, 72], [0, -8], Extrapolation.CLAMP),
    }],
  }));
  ...
  const publicNpub = formatPublicKey(keys?.publicKey);
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
            ...
            <Animated.View entering={FadeInDown.duration(400).delay(100)} style={styles.section}>
              ...
              <View style={[styles.sectionCard, { backgroundColor: colors.elevated, borderColor: colors.border }]}>
                <View style={styles.keySection}>
                  ...
                  <Text style={[styles.keyValue, identityFieldStyle]} numberOfLines={1}>{publicNpub}</Text>
                  <TouchableOpacity
                    style={[styles.copyBtn, identityControlStyle]}
                    onPress={() => copyToClipboard(publicNpub, t('settings.copyPublicKey'))}
                    activeOpacity={0.7}
                  >
      ...
      <Animated.View
        pointerEvents="none"
        style={[styles.headerBackdrop, { height: headerFadeHeight }, headerBackdropStyle]}
      >
        <ProgressiveBlurView
          intensity={20}
        ...
        />
        <LinearGradient
          colors={[colors.background, colors.background, transparentBackground]}
          locations={[0, 0.55, 1]}
          pointerEvents="none"
          style={[styles.statusBarBridge, { height: insets.top + 28 }]}
        />
      </Animated.View>

      <Animated.View
        pointerEvents="none"
        style={[styles.compactHeader, { height: headerHeight, paddingTop: insets.top }, compactHeaderStyle]}
      >
        <Text style={[styles.compactHeaderTitle, { color: colors.text }]}>{t('settings.title')}</Text>
      </Animated.View>
