...
export default function EditNoteScreen() {
  ...
  const flushCurrentEditor = useCallback(async () => {
    const latestContent = (await editorRef.current?.getContent()) ?? contentRef.current;
    contentRef.current = latestContent;
    await flushDraft({ title: titleRef.current, content: latestContent });
  }, [flushDraft]);
  ...
  return (
    <SafeAreaBox style={[styles.container, { backgroundColor: colors.background }]} edges={['left', 'right']}>
      ...
      <KeyboardAvoidingView
        ...
        style={[styles.keyboardAvoider, { paddingTop: headerHeight }]}
      >
      ...
      >
        <View pointerEvents="none" style={[styles.headerBlurLayer, { height: headerFadeHeight }]}>
          <ProgressiveBlurView
            intensity={20}
