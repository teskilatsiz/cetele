...
export default function TechnicalDocumentationScreen() {
  ...
  const { shadows, onScroll: handleShadowScroll } = useScrollShadow({ offset: 8 });

  const scrollHandler = useCallback(
    (event: NativeSyntheticEvent<NativeScrollEvent>) => {
      scrollY.set(event.nativeEvent.contentOffset.y);
      handleShadowScroll(event);
    },
    [handleShadowScroll, scrollY]
  );

  const headerBlurStyle = useAnimatedStyle(() => ({
    opacity: interpolate(scrollY.get(), [0, 60], [0, 1], Extrapolation.CLAMP),
  }));

  const headerBorderStyle = useAnimatedStyle(() => ({
    borderBottomColor: colors.divider,
    borderBottomWidth: interpolate(scrollY.get(), [50, 60], [0, 0.5], Extrapolation.CLAMP),
  }));
    ...
    <SafeAreaBox style={[styles.container, { backgroundColor: colors.background }]} edges={['top']}>
      ...
      <Animated.View style={[styles.header, headerBorderStyle]}>
        <Animated.View style={[StyleSheet.absoluteFill, headerBlurStyle]}>
          <BlurView intensity={80} tint={colors.blurTint} style={[StyleSheet.absoluteFill, styles.headerBlurBg, { backgroundColor: colors.header }]} />
        </Animated.View>
        <TouchableOpacity
        ...
        <Text style={themed.headerTitle}>{d.headerTitle}</Text>
        <View style={styles.headerSpacer} />
      </Animated.View>

      <AnimatedScrollView
        style={styles.scrollView}
        contentContainerStyle={[styles.scrollContent, isTablet && { maxWidth: 1000, alignSelf: 'center' as const }]}
