...
export function BiometricLockScreen({ onUnlock, onAuthenticatingChange }: BiometricLockScreenProps) {
  ...
  useEffect(() => {
    ...
    autoAuthTimerRef.current = setTimeout(() => {
      // Authentication is an external native-system synchronization.
    ...
    // Auto-authentication intentionally starts once for each mounted lock screen.
  ...
  return (
    <SafeAreaBox style={styles.container} edges={['top', 'bottom']}>
      <LinearGradient
        ...
        <Animated.View
          ...
          <Text style={styles.subtitle}>{t('biometric.required')}</Text>

          <View style={styles.authContainer}>
            <View style={styles.authIcon}>
              <ScanFace size={64} color="#0A84FF" strokeWidth={1.5} />
            </View>
