...
export const FormulaInputSheet = memo(function FormulaInputSheet({
  ...
  const isNarrow = width < 460 || fontScale > 1.15;
  ...
  const fieldGroups = useMemo(() => {
    const rows: FormulaField[][] = [];
    let pendingRow: FormulaField[] = [];

    config.fields.forEach((field) => {
      if (isNarrow || field.wide) {
        if (pendingRow.length) rows.push(pendingRow);
        rows.push([field]);
        pendingRow = [];
        return;
      }
  ...
  const fieldRows = fieldGroups.length;
  ...
  return (
    <SwipeableBottomSheet ref={sheetRef} visible={visible} onClose={onClose} keyboardAware>
      {({ maxContentHeight, keyboardVisible }) => {
        const expandedEstimate = 216 + fieldRows * 77 + Math.max(fieldRows - 1, 0) * 12;
        const expandedBodyLimit = Math.max(150, maxContentHeight - 64);
        const compact = keyboardVisible || maxContentHeight < 580 || expandedEstimate > expandedBodyLimit;
        const estimatedBodyHeight = (compact ? 128 : 216) + fieldRows * 77 + Math.max(fieldRows - 1, 0) * 12;
        const naturalBodyHeight = contentHeight > 0 ? contentHeight : estimatedBodyHeight;
        const maxBodyHeight = Math.max(150, maxContentHeight - 64);
        const bodyHeight = Math.max(150, Math.min(naturalBodyHeight, maxBodyHeight));
        const needsScroll = contentHeight > maxBodyHeight + 1;
      ...
      <View style={[styles.contentShell, { maxHeight: maxContentHeight }]}>
        <ScrollView
          ...
          onContentSizeChange={(_, nextHeight) => {
            ...
            setContentHeight((current) => Math.abs(current - roundedHeight) > 1 ? roundedHeight : current);
          }}
        >
          ...
          <View style={styles.fieldsGrid}>
            {fieldGroups.map((row) => (
              <View key={row.map((field) => field.key).join(':')} style={styles.fieldsRow}>
                {row.map((field) => {
                  const fieldIndex = config.fields.findIndex((candidate) => candidate.key === field.key);
                  const nextField = config.fields[fieldIndex + 1];
                  return (
                <View key={field.key} style={[styles.field, row.length === 1 ? styles.fieldWide : styles.fieldHalf]}>
                  <Text
                  ...
                  <TextInput
                    ...
                    onFocus={(event) => {
                      ...
                      if (Platform.OS !== 'web' && needsScroll) {
