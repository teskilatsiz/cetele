...
export class BiometricAuthService {
  private isUnsupportedExpoGoRuntime(): boolean {
    return Platform.OS === 'ios' && isRunningInExpoGo();
  }

  async getBiometricStatus(): Promise<BiometricStatus> {
    if (Platform.OS === 'web' || this.isUnsupportedExpoGoRuntime()) {
      return {
        available: false,
        enrolled: false,
  ...
  async shouldRequireBiometricAuth(): Promise<boolean> {
    if (this.isUnsupportedExpoGoRuntime()) return false;
